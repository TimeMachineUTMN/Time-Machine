...
if (player_in_range && keyboard_check_pressed(ord("E"))) {
	dialogue_texts = [
    "Oh, it`s does not work", 
    "Seems I need to find 7 gears",
	"Found " + string(global.gear_count) +"/7"
]; // Массив текстов для диалога
	if global.gear_count ==7{
	
                dialogue_texts=["Congratulations, you won!"]
				instance_destroy(inst_3C7D58D);
				instance_destroy();
	}
